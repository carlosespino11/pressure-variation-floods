...
```{r pca_interactive_map, echo = FALSE, message =FALSE, warning= FALSE, fig.height=800}
library(shiny)
library(leaflet)
...
This map shows the pressure variations by year for measurement in space versus the floods that ocurred during that year. The color of the circles represent the magnitude of the glood and the size represent the number of affect squared kilometers, *we can click on the balls to get more information about the flood*. The variations in pressure were calculated using PCA analysis and taking the first eigenvectors, remember that the eigenvectors represent the directions of more variance.

Also the map shows some complementary plots that summarize the floods throughout the year.

It can be appreciated, in general, that in places were pressure variatons  are higher, the size of the floods is bigger in both, severity and squared kilometers affected. These changes in air pressure may give us some informations about the storms.

The auxiliary plots show that the floods are cause mainly by storms and there is no clear trend in the number of floods and magnitude by month on a specific year.
